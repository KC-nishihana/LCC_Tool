...
5.  インポート設定を調整します：
    ...
    *   **Render Distance**: 距離による表示密度減衰の強さを調整します。
...
*   **2D投影**: 透視投影のヤコビアン行列 (Jacobian matrix) を使用して、3D楕円体を2D楕円に変換
...
**頂点シェーダー**:
*   クォータニオンから回転行列への変換
*   スケール行列との結合による変換行列Mの構築
*   3D共分散行列Vの計算 (V = M × M^T、Vは共分散行列)
*   透視投影のヤコビアン行列の計算
*   2D画面空間における共分散行列の導出
*   楕円の固有値分解と主軸の計算
*   適応的なクアッド頂点の配置
...
**フラグメントシェーダー**:
*   半径ベースのガウス減衰計算：α = exp(-0.5 × r² × 9)（9 = 3² は3σルールに基づく正規化係数）
*   アルファブレンディングの最適化
